...
namespace Lab1
{
    class FileDataSource : IDataSource
    {
        ...
        public List<Document> GetLegals()
        {
            ...
            {
            using (FileStream fs = new FileStream(FilePathForLegal, FileMode.Open))
            {

                for (int i = 0; i < IndexAdministrative; i++)
                {
                    documents.Add((Document)binaryFormatter.Deserialize(fs));
                }
            }
                return documents;
            }
        ...
        public List<Document> GetAdministatives()
        {
            List<Document> documents = new List<Document>();
            BinaryFormatter binaryFormatter = new BinaryFormatter();
            using (FileStream fs = new FileStream(FilePathForAdmin, FileMode.Open))
            {
